...
calcresp <- read.csv("C:/Users/Jessica Briggs/Box/Dissertation/Sondes & Ponds/Intermediate Data Products/Storm Quantitative Responses.csv")
...
rain <- read.csv('./Meteo Data/CoCoRaHS Data Clean.csv') %>%
...
tchain <- read.csv('./Sensors/Temp Pendants/TChain Full Dataset.csv') %>%
...
depth <- read.csv('./Sensors/Water Level Loggers/Water Depth Manual Calculations Full Dataset.csv') %>%
  mutate(datetime = as.POSIXct(datetime)) %>%
  filter(pond %in% c('Strickers','Tiedemans'))
...
day22 <- read.csv("./Meteo Data/Middleton Daylight 2022.csv") %>%
...
day23 <- read.csv("./Meteo Data/Middleton Daylight 2023.csv") %>%
  pivot_longer(Jan.:Dec., names_to = "month", values_to = 'daylength') %>%
  filter(daylength != '') %>%
  separate_wider_delim(daylength, delim = ':', names = c('hours','minutes')) %>%
  mutate(daylength_hr = as.numeric(hours) + as.numeric(minutes)/60,
...
day24 <- read.csv("./Meteo Data/Middleton Daylight 2024.csv") %>%
  pivot_longer(Jan.:Dec., names_to = "month", values_to = 'daylength') %>%
  filter(daylength != '') %>%
  separate_wider_delim(daylength, delim = ':', names = c('hours','minutes')) %>%
  mutate(daylength_hr = as.numeric(hours) + as.numeric(minutes)/60,
...
#Read in Lab Data
chem <- read.csv('./Laboratory Data/Chem Masters/Ponds Chemistry Jan 2025.csv') %>%
...
write.csv(stormchem, "C:/Users/Jessica Briggs/Box/Dissertation/Sondes & Ponds/Intermediate Data Products/Possible Pre or Post Storm Chemistry Samples.csv", row.names = F)

#Read back in those deemed true storm samples that had a pair
stormchem <- read.csv("C:/Users/Jessica Briggs/Box/Dissertation/Sondes & Ponds/Intermediate Data Products/Confirmed Pre or Post Storm Chemistry Samples.csv") %>%
...
poststormchem <- read.csv("C:/Users/Jessica Briggs/Box/Dissertation/Sondes & Ponds/Intermediate Data Products/Confirmed Post Storm Chemistry Samples.csv") %>%
  mutate(g440 = (2.303*x440)/(0.01)) %>%
...
write.csv(join7, "C:/Users/Jessica Briggs/Box/Dissertation/Sondes & Ponds/Intermediate Data Products/Quantitative Pigment Responses and Potential Explanatory Variables.csv", row.names = F)
